...
export function buildCustomSakanaResponsesUrl(accountId = DEFAULT_CF_AI_GATEWAY_ACCOUNT_ID, gatewayId = DEFAULT_CF_AI_GATEWAY_ID) {
...
function extractStreamDeltaIncremental(payload) {
...
function extractStreamDeltaFallback(payload) {
    const t = payload.type;
    if (typeof t !== 'string')
        return '';
...
function enqueueParsedSseLine(data, controller, sawOutputTextDelta) {
    if (!data || data === '[DONE]')
        return;
    try {
...
export function openAiResponsesEventStreamToText() {
    let buffer = '';
...
export async function completeViaGatewayResponses(config, messages, maxOutputTokens, stream) {
    const headers = {
    ...
    if (!res.body)
        throw new Error('Fugu gateway stream missing body');
    return res;
}
...
export async function streamViaGatewayResponses(config, messages, maxOutputTokens) {
