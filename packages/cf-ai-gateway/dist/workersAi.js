 ...
 */
export function resolveWorkersAiChatModel(audreyModel) {
...
function clampMaxTokens(maxCompletionTokens) {
    const requested = maxCompletionTokens ?? DEFAULT_WORKERS_AI_MAX_COMPLETION_TOKENS;
 ...
 */
export function buildWorkersAiChatInput(messages, maxCompletionTokens, stream) {
    return {
        messages,
        stream,
 ...
 */
export async function streamViaWorkersAiChat(ai, model, messages, maxCompletionTokens) {
    const result = await ai.run(model, buildWorkersAiChatInput(messages, maxCompletionTokens, true));
