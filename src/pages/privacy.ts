...
function zhSection(cssClass: string): string {
  return `<section${cssClass} aria-labelledby="privacy-zh">
...
export function renderPrivacyPolicyPage(lang: PageLang = 'zh-Hant'): string {
  ...
  const first = lang === 'en' ? enSection('') : zhSection('')
  const second = lang === 'en' ? zhSection(' class="lang"') : enSection(' class="lang"')
  return `<!DOCTYPE html>
<html lang="${lang}">
<head>
  <meta charset="utf-8">
