// 受信任呼叫端的 Bearer token 驗證——鏡像自姊妹專案 sayit-hono 的 src/api/auth.ts。
//
// constant-time（不依賴第幾個 token 命中）的 SHA-256 雜湊比對。把對 `===` /
// `!==` 的時間差攻擊面降到只剩 hash 一致時的 byte-XOR fold——而 fold 走滿整段
// 32 bytes 不 short-circuit。
//
// 為什麼不是直接 string compare：JS 字串相等比對在 V8 / workerd 上可能依長度
// 短路、依首字元短路。理論上的 timing 訊息都不應該洩漏給攻擊者，即使網路
// jitter 通常會蓋掉這個差。
...
// Header 缺失或不是 Bearer 直接 false——那一路徑與 token 比對 timing 沒關係
// （攻擊者必須先給 valid Bearer 格式才會走到 hash 比對）。
export async function isAuthorizedFromHeader(
  ...
  return verifyTranscriptToken(token, ...allowed)
}
